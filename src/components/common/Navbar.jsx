...
const Navbar = () => {
  ...
  return (
    <div className="w-full flex justify-center px-4 mt-6 mb-4 fixed top-0 z-50 pointer-events-none">
      ...
      <nav className={`w-full max-w-6xl bg-white/70 backdrop-blur-xl border border-white/50 rounded-3xl shadow-[0_8px_32px_rgba(0,0,0,0.05)] transition-all duration-300 pointer-events-auto relative ${
        scrolled ? 'py-3 px-6 shadow-[0_8px_32px_rgba(0,0,0,0.08)]' : 'py-4 px-6'
      }`}>
        
        <div className="flex justify-between items-center gap-2 md:gap-6">
          
          {/* Logo */}
          <Link to="/" className="text-xl md:text-2xl font-bold text-[#3B82F6] tracking-tight whitespace-nowrap hover:scale-105 transition-transform duration-300">
            MarketPlace
          </Link>
          ...
          <button 
            ...
            className="md:hidden text-gray-600 text-2xl p-2 hover:bg-white/60 rounded-full transition-all duration-300"
          >
          ...
          <div className="flex items-center gap-2 md:gap-3">
            {/* ប្រើ FiShoppingCart ផ្ទាល់ជំនួសឱ្យ <Cart /> */}
            <Link to="/cart" className="p-2.5 hover:bg-white/60 rounded-full transition-all duration-300 hover:shadow-md">
              <FiShoppingCart className="text-xl text-gray-600 transition-colors duration-300 hover:text-[#3B82F6]" />
            </Link>
            
            <Link to="/wishlist" className="relative p-2.5 hover:bg-white/60 rounded-full transition-all duration-300 hover:shadow-md">
              <FiHeart className="text-xl text-gray-600 transition-colors duration-300 hover:text-[#3B82F6]" />
              {wishlistCount > 0 && (
                <span className="absolute -top-0.5 -right-0.5 bg-[#E23D3D] text-white rounded-full w-5 h-5 flex items-center justify-center text-[10px] font-bold border-2 border-white animate-pulse shadow-sm">
                  {wishlistCount}
                </span>
              )}
            </Link>
            ...
            {isAuthenticated ? (
              <div className="relative group hidden md:block">
                <button className="p-2.5 hover:bg-white/60 rounded-full transition-all duration-300 hover:shadow-md">
              ...
              <Link to="/login" className="p-2.5 hover:bg-white/60 rounded-full transition-all duration-300 hover:shadow-md hidden md:block">
                <FiUser className="text-xl text-gray-600 transition-colors duration-300 hover:text-[#3B82F6]" />
              </Link>
            )}
          </div>
        ...
        {mobileMenuOpen && (
          <div className="md:hidden absolute top-full left-0 right-0 mt-3 bg-white/90 backdrop-blur-md rounded-2xl shadow-xl border border-white/50 p-4 flex flex-col gap-3 animate-fadeInUp z-50">
            <Link to="/" onClick={handleMobileLinkClick} className="px-4 py-2 hover:bg-[#3B82F6]/10 rounded-xl text-gray-700 transition-colors duration-200 font-medium text-sm">
              ទំព័រដើម
            </Link>
            <Link to="/shop" onClick={handleMobileLinkClick} className="px-4 py-2 hover:bg-[#3B82F6]/10 rounded-xl text-gray-700 transition-colors duration-200 font-medium text-sm">
              ហាង
            </Link>
            <Link to="/orders" onClick={handleMobileLinkClick} className="px-4 py-2 hover:bg-[#3B82F6]/10 rounded-xl text-gray-700 transition-colors duration-200 font-medium text-sm">
              ការបញ្ជាទិញ
            </Link>
            ...
            <div className="border-t border-gray-100/80 mt-2 pt-3 px-2">
              {isAuthenticated ? (
                <div className="flex flex-col gap-2">
                  <Link to="/profile" onClick={handleMobileLinkClick} className="px-4 py-2 hover:bg-[#3B82F6]/10 rounded-xl text-gray-700 transition-colors duration-200 text-sm">
                    ប្រវត្តិរូប
                  </Link>
                  <button 
                    onClick={() => { logout(); handleMobileLinkClick(); }} 
                    className="px-4 py-2 hover:bg-red-50/80 rounded-xl text-red-500 transition-colors duration-200 text-sm text-left"
                  >
                    ចេញ
                  </button>
                </div>
              ) : (
                <Link to="/login" onClick={handleMobileLinkClick} className="block px-4 py-2 bg-[#3B82F6] text-white text-center rounded-xl hover:bg-blue-600 transition-colors duration-200 text-sm font-medium">
                  ចូលប្រើប្រាស់
                </Link>
              )}
            </div>
